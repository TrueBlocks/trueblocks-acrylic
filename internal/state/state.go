...
type UIPreferences = appkit.UIPreferences
...
type AppState struct {
	...
	UIPrefs      appkit.UIPreferences         `json:"uiPrefs,omitempty"`
...
func NewManager() *Manager {
	statePath := filepath.Join(appkit.AppDirFor("acrylic"), "state.json")
...
func (m *Manager) GetThemeState() appkit.UIPreferences {
	s := m.store.Get()
	prefs := s.UIPrefs
	if prefs.Theme == "" {
		prefs.Theme = "default"
...
func (m *Manager) SetThemeState(prefs appkit.UIPreferences) {
	_ = m.store.Update(func(s *AppState) {
		s.UIPrefs = prefs
	})
}
